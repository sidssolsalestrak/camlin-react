...
import {
    Box, Grid, FormControl, InputLabel, Select, MenuItem, Button,
    Dialog, DialogActions, DialogTitle, DialogContent, Divider,
    Typography, IconButton, Checkbox, Tooltip, Paper, Chip,
    TextField, RadioGroup, FormControlLabel, Radio, Autocomplete,
    Backdrop, CircularProgress, Switch,
} from "@mui/material";
...
function UploadClosing() {
    ...
    const [mapModal, setMapModal] = useState({ open: false, row: null, suggestions: [] });
        ...
        const pages = (data.result || []).map((page) => {
            if (formatType === 1) {
                ...
                for (let l = 0; l < page.length; l++) {
                    if (page[l] && page[l][0] === "Range") { headerRowIdx = 1; break; }
                }
                const colCount = Math.min(4, (page[0] || []).length);
                const headers = Array.from({ length: colCount }, (_, i) => page[headerRowIdx]?.[i] ?? "");
                const rows = [];
    ...
    useEffect(() => {
        (async () => {
            ...
            } catch (err) { console.error("fetchDesname:", err); }
        })();
    }, []);
    ...
    const loadDesListData = useCallback(async () => {
        ...
        } catch (err) { console.error("fetchDesList:", err); }
    ...
    const handleImport = async () => {
        if (!selDesName || selDesName === "0") { toast.error("Please select a Distributor."); return; }
        if (files.length === 0) { toast.error("Please select a file to import."); return; }
        const [desId, desName, desCode] = selDesName.split("|");
        const form = new FormData();
        files.forEach((f) => {
            const ext = f.name.split(".").pop().toLowerCase();
            form.append(["xlsx", "xls"].includes(ext) ? "uploadFile" : "uploadFile[]", f);
        });
    ...
    const handleAddManual = async (overrideTglVal) => {
        const desId = selDesName.split("|")[0];
        if (!desId || desId === "0") { toast.error("Please select a Distributor."); return; }
        ...
        try {
            ...
            setManualMode(true);
            handleApiResponse(res.data);
        } catch (err) { console.error("addManual:", err); }
        finally { setLoading(false); }
    };
    ...
    const handleRawMappingChange = (pageIdx, colIdx, value) => {
        setRawPages((prev) => prev.map((page, pIdx) => {
    ...
    const handleRawCellChange = (pageIdx, rowIdx, colIdx, value) => {
        setRawPages((prev) => prev.map((page, pIdx) => {
            ...
            const rows = page.rows.map((r, rIdx) =>
                rIdx !== rowIdx ? r : r.map((c, cIdx) => (cIdx === colIdx ? value : c))
            );
            return { ...page, rows };
        }));
    };
                ...
                renderHeader: () => (
                    <Select
                        ...
                        onChange={(e) => {
                            ...
                            handleRawMappingChange(pageIdx, colIdx, v === "_original" ? null : v);
                        }}
                        sx={{ fontSize: 12, background: "#fff", "& .MuiSelect-select": { py: "4px" } }}
                    >
                        <MenuItem value="_original" sx={{ fontSize: 12 }}>{label || `Column ${colIdx + 1}`}</MenuItem>
                        <MenuItem value="code" sx={{ fontSize: 12 }}>Code</MenuItem>
                        <MenuItem value="product_name" sx={{ fontSize: 12 }}>Product Name</MenuItem>
                        <MenuItem value="closing_qty" sx={{ fontSize: 12 }}>Closing Qty</MenuItem>
                    </Select>
                ),
                renderCell: ({ row }) => (
                    <TextField
                        ...
                        onChange={(e) => handleRawCellChange(pageIdx, row._rowIdx, colIdx, e.target.value)}
    ...
    const rawRowsForTable = (page) =>
        page.rows.map((cells, idx) => ({ id: idx, _rowIdx: idx, _sl: idx + 1, cells }));

    const handleRawSubmit = () => {
        for (let p = 0; p < rawPages.length; p++) {
            const mapped = Object.values(rawPages[p].mapping);
            if (!mapped.includes("product_name")) { toast.error(`Please select Product Name in page ${p + 1}`); return; }
            if (!mapped.includes("closing_qty")) { toast.error(`Please select Closing Qty in page ${p + 1}`); return; }
        }
        ...
        for (let p = 0; p < rawPages.length; p++) {
            ...
            const productIdx = Number(Object.keys(page.mapping).find((k) => page.mapping[k] === "product_name"));
            const qtyIdx = Number(Object.keys(page.mapping).find((k) => page.mapping[k] === "closing_qty"));
            const codeKey = Object.keys(page.mapping).find((k) => page.mapping[k] === "code");
            ...
            onConfirm: async () => {
                ...
                try {
                    const res = await api.post("/upload_to_db", {
                        des_id: desId, des_name: desName, des_code: desCode,
                        selected_mnt: parseMonth(selMonth),
                        allValues, tot_page: totPage,
                    });
                    setRawPages([]);
                    setManualMode(false);
                    ...
                    handleApiResponse(res.data);
                } catch (err) { console.error("uploadToDb:", err); }
                finally { setLoading(false); }
            },
        });
    };
    ...
    const handleQtyChange = (rowKey, val) => {
        setTableData((prev) =>
            prev.map((r) => r._rowKey === rowKey ? { ...r, prod_qty: val.replace(/\D/g, "") } : r)
        );
    };
    ...
    const handleMapInputChange = (val) => {
        ...
        if ((val || "").length < 2) { setMapAutoResults([]); return; }
        mapDebounce.current = setTimeout(async () => {
            ...
            } catch { setMapAutoResults([]); }
        }, 300);
    };

    const handleDoMap = async () => {
        const selected = mapRadio || mapAutoSelected;
        if (!selected) { toast.error("Please select at least one option."); return; }
        const { row } = mapModal;
    ...
    const handleDeleteRow = (row) => {
        setConfirm({
            ...
            onConfirm: async () => {
                closeConfirm();
                try {
                    let response = await api.post("/delete_single_prod", { rowId: row.id, mas_id: row.mas_id });
    ...
    const handleDeleteSelected = () => {
        ...
        if (!keys.length) { toast.error("Please select at least one product."); return; }
        setConfirm({
            open: true,
            title: "Confirmation",
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    let response = await api.post("/selected_dlt", { dlt_prod_id: ids, prod_mas_id: masIds });
    ...
    const handleAbort = () => {
        setConfirm({
            ...
            onConfirm: async () => {
                try {
                    let response = await api.post("/delete_all", { mas_id: masId, json_name: jsonName });
                    setTableData([]); setMasId(null); setFiles([]);
                    setImgData([]); setFileType(null); setPreviewFile(null);
                    setRawPages([]);
                    setManualMode(false);
                    if (response.data.status === 200) {
                        toast.success(response.data.message)
    ...
    const handleManualInsert = () => {
        ...
        setConfirm({
            ...
            onConfirm: async () => {
                ...
                const hidProdNameStr = tableData.map((r) => r.prod_name ?? "").join(",");
                ...
                try {
                    ...
                    await loadDesListData();
                } catch (err) { console.error("insertManual:", err); }
                finally {
                    closeConfirm()
                }
            },
    ...
    const handleConfirm = () => {
        if (!validateQuantities(tableData)) return;
        const allData = tableData.map((r) => ({
            product_id: r.id, prod_id: r.prod_id, mas_id: r.mas_id,
            prod_name: r.prod_name, prod_code: r.prod_code, prod_qty: r.prod_qty,
        }));
        setLoading(true);
        (async () => {
            try {
                const validRes = await api.post("/final_submit_validation", { allData });
                    ...
                    confirmationText = (
                        <>
                            ...
                            {(validRes.data.data || []).map((p, i) => (
                                <div key={i}>{p.prod_code} - {p.prod_name}</div>
                            ))}
                        </>
                    );
                }
    ...
    const handleDltThisData = async () => {
        setConfirm({
            ...
            onConfirm: async () => {
                try {
                    let response = await api.post('/dlt_All', { mas_id: masId, json_name: jsonName })
                    if (response.data.status === 200) {
                        toast.success(response.data.message)
                    } else {
                        toast.error("someThing went wrong,try again!")
    ...
    const filteredRows = useMemo(() => {
        let rows = tableData;
        if (activeFilter === "mapped") rows = rows.filter((r) => r.prod_map_stat !== 1 && r.qty_map_stat !== 1);
        ...
        if (isApproved && numbered.length > 0) {
            const grandTotal = numbered.reduce((sum, r) => sum + (Number(r.prod_qty) || 0), 0);
            numbered.push({
            ...
            renderCell: ({ row }) => {
                ...
                return (
                    <TextField
                        ...
                        inputProps={{ style: { textAlign: "center", fontSize: 13, width: 64 } }}
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, flexWrap: "wrap" }}>
                            ...
                            {!isApproved &&
                                <Tooltip title="Map product">
                                    <IconButton size="small" onClick={() => handleOpenMapModal(row)} sx={{ color: "primary.main", p: "2px" }}>
                            ...
                            <Typography
                                ...
                                sx={{
                                    fontSize: 12,
                                    cursor: "pointer",
                                    color: row.strak_prod_name == null && row.pn === '' ? "error.main" : "text.primary",
                                    "&:hover": { color: !isApproved ? "primary.main" : null, textDecoration: !isApproved ? "underline" : null },
                                }}
                            >
                                {row.cat_code_1} {row.prod_code ? `| ${row.prod_code}` : ""} | {row.prod_name}
                            </Typography>
                            {row.pn && <Checkbox size="small" defaultChecked sx={{ p: "2px" }} />}
                        </Box>
                        {!isApproved ?
                            <Box sx={{ ml: 2.5 }}>
                                {(row.strak_prod_name) ? (
                                    <Typography variant="caption">
                                        <Box component="span" color="text.secondary">mapped as: </Box>
                                        <Box component="span" color="success.main">{row.strak_prod_code} | {row.strak_prod_name}</Box>
                                    </Typography>
                                ) : (
                                    <Typography variant="caption">
                                        <Box component="span" color="text.secondary">suggestions: </Box>
                                        <Box component="span" color="error.main">{row.pc && row.pn ? `${row.pc} | ${row.pn}` : ""}</Box>
                                    </Typography>
                                )}
                            </Box> : null
                        }
            ...
            renderHeader: () => (
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                    <Tooltip title="Delete selected">
                        <IconButton size="small" color="error" onClick={handleDeleteSelected} sx={{ p: "2px" }}>
                            <FaTrashAlt size={16} />
                        </IconButton>
                    </Tooltip>
                ...
                return (
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0.5 }}>
                        <Tooltip title="Delete">
                            <IconButton size="small" onClick={() => handleDeleteRow(row)} sx={{ color: "action.disabled" }}>
                                <FaTrash size={16} />
                            </IconButton>
                        </Tooltip>
                        <Checkbox
                            ...
                            onChange={(e) => setDltChecked((prev) => ({ ...prev, [row._rowKey]: e.target.checked }))}
                            sx={{ p: "2px" }}
                        />
                    </Box>
                );
    ...
    const legendChips = [
        { key: "mapped", color: MAP_COLORS.mapped, label: "Mapped", cnt: counts.mapped },
        { key: "semi", color: MAP_COLORS.semi, label: "Semi-Mapped", cnt: counts.semi },
    ...
    return (
        ...
        >
            <Box p={2} display="flex" flexDirection="column" gap={2}>
                ...
                <Paper elevation={0} sx={{ p: "16px 18px", borderRadius: "10px", boxShadow: "0 1px 3px rgba(0,0,0,0.07), 0 4px 12px rgba(0,0,0,0.04)" }}>
                    <Grid container spacing={1.5} alignItems="flex-end">
                        ...
                        <Grid size={{ xs: 12, sm: 6, md: 3, lg: 3 }}>
                            <Box sx={{ display: "flex", flexDirection: "column", height: "35px" }}>
                                <FormControl size="small" fullWidth>
                                    ...
                                    <Select
                                        ...
                                        value={selDesName}
                                        onChange={(e) => {
                                            setSelDesName(e.target.value);
                                            setManualMode(false);
                                        }}
                                        disabled={checking === 2}
                                        MenuProps={{ PaperProps: { style: { maxHeight: 200 } } }}
                                    >
                                        <MenuItem value="0">Select Distributor name</MenuItem>
                                        {allDesname.map((d) => (
                                            <MenuItem key={d.id} value={`${d.id}|${d.stk_name}|${d.stk_code}|${d.ter_name}`}>
                                                {d.stk_name} – {d.stk_code}
                                            </MenuItem>
                                        ))}
                                    </Select>
                                </FormControl>
                                {showDistributorTerritory && (
                                    <Typography variant="caption" sx={{ mt: 0.1, color: "#212121", fontSize: "12px" }}>
                                        ...
                                        <Box display="flex" flexWrap="wrap" gap={0.5}>
                                            {files.map((f) => (
                                                <Chip
                                                    ...
                                                    label={f.name}
                                                    size="small"
                                                    onDelete={() => setFiles((p) => p.filter((x) => x.name !== f.name))}
                                                    sx={{ fontSize: 11 }}
                                                />
                                            ))}
                                        </Box>
                        ...
                        {hasPreviewFiles && (
                            <Grid size={{ xs: 12, sm: "auto" }} sx={{ display: "flex", alignItems: "center" }}>
                                <Tooltip title="Preview uploaded file">
                                    <IconButton
                                        onClick={() => setPreviewFile({ docName: imgData[0]?.doc_name ?? "", fileType })}
                                        sx={{
                                            border: "1.5px solid",
                        ...
                        {processStat !== null && (
                            ...
                            >
                                <Box display="flex" alignItems="center" gap={1}>
                                    {(isPending || isApproved || isRawPending || isRejected) && <Typography variant="caption" fontSize={12} fontWeight={500}>STATUS:</Typography>}
                                    {isPending && (
                                        <Typography variant="caption" fontSize={12} sx={{ display: "flex", alignItems: "center", gap: 0.5, color: "success.main", fontWeight: 500, textWrap: "nowrap" }}>
                                            Unconfirmed (In Process)
                                        </Typography>
                                    )}
                                    {isRawPending && (
                                        <Typography variant="caption" fontSize={12} sx={{ display: "flex", alignItems: "center", gap: 0.5, color: "warning.main", fontWeight: 500, textWrap: "nowrap" }}>
                                            In Pending <InsertDriveFileIcon sx={{ fontSize: 12 }} />
                                        </Typography>
                                    )}
                                    {isRejected && (
                                        <Typography variant="caption" fontSize={12} sx={{ display: "flex", alignItems: "center", gap: 0.5, color: "error.main", fontWeight: 500 }}>
                                            Rejected <MdBlockFlipped size={12} />
                                        </Typography>
                                    )}
                                    {isApproved && (
                                        <Typography variant="caption" fontSize={12} sx={{ display: "flex", alignItems: "center", gap: 0.5, color: "success.main", fontWeight: 500 }}>
                                            Approved <FaThumbsUp size={12} />
                                        </Typography>
                                    )}
                                    {!isPending && !isRawPending && !isRejected && !isApproved && (
                                        <Typography variant="caption" fontSize={12} sx={{ color: "text.secondary", fontWeight: 500 }}>
                                            {processStat}
                                        </Typography>
                                    )}
                                </Box>
                ...
                {rawMode && currentRawPage && (
                    <Paper elevation={0} sx={{ borderRadius: "10px", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.07), 0 4px 12px rgba(0,0,0,0.04)" }}>
                        <Box sx={{ p: "10px 14px", borderBottom: "1px solid", borderColor: "divider", display: "flex", flexWrap: "wrap", gap: 1, alignItems: "center" }}>
                            <Box sx={{ flex: 1 }} />
                            {rawPages.length > 1 && (
                                <Box sx={{ display: "flex", gap: 0.5 }}>
                                    <Button size="small" variant="outlined" disabled={rawPageIndex === 0} onClick={() => setRawPageIndex((i) => i - 1)}>‹</Button>
                                    {rawPages.map((_, i) => (
                                        <Button
                                            key={i}
                                            size="small"
                                    ...
                                    <Button size="small" variant="outlined" disabled={rawPageIndex === rawPages.length - 1} onClick={() => setRawPageIndex((i) => i + 1)}>›</Button>
                                </Box>
                            )}
                            {masId && canDelete && (
                ...
                {showTable && (
                    <Paper elevation={0} sx={{ borderRadius: "10px", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.07), 0 4px 12px rgba(0,0,0,0.04)" }}>
                        ...
                        <Box sx={{ p: "10px 14px", borderBottom: "1px solid", borderColor: "divider", display: "flex", flexWrap: "wrap", gap: 0.75, alignItems: "center" }}>

                            {legendChips.map(({ key, color, label, cnt }) => (
                                <Chip
                                    key={key}
                                    icon={<Box sx={{ width: 10, height: 10, borderRadius: "2px", background: color, ml: "6px !important" }} />}
                                    label={
                                        <>
                                            {label}
                                            {cnt > 0 && (
                                                <Box component="span" sx={{ fontWeight: 600, color, ml: 0.5 }}>({cnt})</Box>
                                            )}
                                        </>
                                    }
                                    onClick={() => setActiveFilter(activeFilter === key ? "total" : key)}
                                    variant={activeFilter === key ? "outlined" : "filled"}
                                    sx={{
                                        borderColor: activeFilter === key ? color : "transparent",
                                        background: activeFilter === key ? `${color}18` : "action.hover",
                            ...
                            <Box
                                ...
                                color={activeFilter === "total" ? "primary.main" : "text.secondary"}
                                sx={{
                                    "&:hover": { color: "primary.main !important", textDecoration: "underline", cursor: 'pointer' },
                                }}
                            >
                            ...
                            {(isPending || isRejected) &&
                                <Box onClick={handleDltThisData} sx={{ display: 'flex', cursor: 'pointer' }}>
                                    <FaTrash size={15} color="red" />
                                    <Typography sx={{ textWrap: 'wrap' }}>Delete this Data</Typography>
                                </Box>}
                            <Box sx={{ flex: 1 }} />
                            ...
                            {!isApproved && !isRejected && !manualMode && (
                                <Tooltip title={canConfirm ? "Confirm and submit" : "Resolve all mapping issues first"}>
                                    <span>
                                        <Button
                                            size="small"
                                            variant="contained"
                            ...
                            {canUpdateApproved && !manualMode && (
                                <Button size="small" variant="contained" startIcon={<SaveIcon />} onClick={handleManualInsert} disabled={loading}>
                                    Update
                                </Button>
                            )}
                        ...
                        <DataTable
                            ...
                            rowStyle={rowStyle}
                            getRowClassName={(params) => params.row._isGrandTotal ? "grand-total-row" : ""}
                            sx={{
                                "& .grand-total-row": {
                ...
                {isLastRawPage && rawMode && (
                    <Box sx={{ display: "flex", flexDirection: "row", width: "100%", justifyContent: "end" }}>
                        <Button sx={{ width: "2rem" }} variant="contained" onClick={handleRawSubmit} disabled={loading}>
                            Submit
                        </Button>
                    </Box>
                )}
                ...
                {!showTable && !rawMode && !loading && selDesName !== "0" && (
                    <Paper elevation={0} sx={{ p: 5, textAlign: "center", borderRadius: "10px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
            ...
            <Dialog open={mapModal.open} onClose={() => setMapModal((m) => ({ ...m, open: false }))} maxWidth="xs" fullWidth>
                <DialogTitle sx={{ pb: 0 }}>
                    <Typography variant="caption" color="text.secondary">Mapping Product</Typography>
                    <Typography fontWeight={500} fontSize={15}>{mapModal.row?.prod_name}</Typography>
                    <IconButton
                ...
                <DialogContent sx={{ pt: 1.5 }}>
                                    ...
                                    <FormControlLabel
                                        ...
                                        control={
                                            <Radio
                                                size="small"
                                                onChange={() => {
                                                    setMapRadio({ id: s.prod_id, code: s.prod_code, name: s.prod_name });
                                                    setMapAutoSelected(null);
                                                }}
                                            />
                                        }
                                        label={<Typography fontSize={13}>{s.prod_code} – {s.prod_name}</Typography>}
                                        sx={{ my: 0 }}
                                    />
                                ))}
                            </RadioGroup>
                    ...
                    <Typography variant="caption" color="text.secondary" fontWeight={500} sx={{ mb: 0.75, display: "block" }}>
                        Others
                    </Typography>
                    <Autocomplete
                        freeSolo
                        options={mapAutoResults}
                        getOptionLabel={(o) => typeof o === "string" ? o : `${o.prod_code} – ${o.prod_name}`}
                        onInputChange={(_, val) => handleMapInputChange(val)}
                        onChange={(_, val) => {
                            if (val && typeof val === "object") {
                                setMapAutoSelected({ id: val.prod_id, code: val.prod_code, name: val.prod_name });
                                setMapRadio(null);
                            }
                        }}
                        renderInput={(params) => (
                            <TextField {...params} size="small" placeholder="Search product…" fullWidth />
                        )}
                    />
                </DialogContent>
                <Divider />
                <DialogActions sx={{ px: 3, py: 1.5 }}>
                    <Button variant="outlined" onClick={() => setMapModal((m) => ({ ...m, open: false }))}>Close</Button>
                    <Button variant="contained" startIcon={<LinkIcon />} onClick={handleDoMap}>Map</Button>
                </DialogActions>
            </Dialog>
