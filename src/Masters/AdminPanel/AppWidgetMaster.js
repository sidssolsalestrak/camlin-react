...
export default function AppWidgetMaster() {
  ...
  const decodedWidgetId =
    editwidgetId !== undefined && editwidgetId !== null
      ? Number(atob(editwidgetId))
      : null;
  ...
  const fetchAppWidgetData = async () => {
    ...
    try {
      ...
      let widgetRes = Array.isArray(response.data.data)
        ? response.data.data
        : [];
      setAllWidgetData(
        widgetRes.map((item, index) => ({ ...item, si_no: index + 1 })),
      );
    } catch (err) {
      console.log(err);
    } finally {
  ...
  const fetchFieldWidgetData = async () => {
    try {
      ...
      let userInputData = Array.isArray(response.data.userResData)
        ? response.data.userResData
        : [];
      let widgetResData = Array.isArray(response.data.widgetMenuRes)
        ? response.data.widgetMenuRes
        : [];
  ...
  const validateFields = () => {
    ...
    if (!selUserInput || Number(selUserInput?.id) === 0) {
      // ← check null
      setUserTypeErr(true);
      isValid = false;
    }
    ...
    if (selWidgetMenu.length === 0) {
      toast.error(
        `Please Select atleast one ${masterPanel["AWMS"] || "Menu"} option !`,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          decodedWidgetId
            ? `${masterPanel["AWMS"] || "App Widget Menu"} Updated successfully`
  ...
  const collectEditData = async (userId) => {
    try {
      ...
      const matchedUser =
        allUserInputData.find((u) => u.id === data[0].user_type) || null;
      ...
      setOriginalWidgetMenu(menuArrData);
      setTabValue(0);
    } catch (err) {
      console.log(err);
    }
  ...
  const isEdited = decodedWidgetId
    ? JSON.stringify([...selWidgetMenu].sort((a, b) => a - b)) !==
      JSON.stringify([...originalWidgetMenu].sort((a, b) => a - b))
    : true;
  ...
  return (
    <Layout
      breadcrumb={[
        ...
        {
          label: masterPanel["AWMS"] || "App Widget Master",
      ...
      >
        <Box>
          <h1 className="mainTitle">
            {masterPanel["AWMS"] || "App Widget Master"}
          </h1>
        </Box>
        ...
        >
          {!decodedWidgetId ? (
            ...
            >
              Edit {masterPanel["AWMS"] || "App Widget"}
            </Typography>
          )}
          {tabValue === 0 && (
            <Box
            ...
            >
              ...
              <Autocomplete
                options={[
                  {
                    id: "0",
                    client_alias: `Select ${masterPanel["USER"] || "User"} Type`,
                  },
                  ...allUserInputData,
                ]}
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    sx={{
                      backgroundColor: decodedWidgetId ? "#EEEEEE" : undefined,
                    }}
                  />
                )}
              />
              ...
              <Button
                variant="contained"
                sx={{ width: "2rem", mb: 3, textTransform: "none" }}
                disabled={!!decodedWidgetId && !isEdited}
                onClick={() => {
